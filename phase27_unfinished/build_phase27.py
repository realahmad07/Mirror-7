...
def repair_find_word_targets(lines):
    ...
    for line in lines:
        ...
        if toks and toks[0]==':' and toks[1]=='find-word':
            ...
            boundary_exit_pc=pcs[success_exit_i]
            candidate_advance_pc=limit_start
            char_loop_branch_i=body.index('branch:390')
            for i,t in enumerate(body):
                if not t.startswith('0branch:'):continue
                if i<name_len_i: body[i]=f'0branch:{boundary_exit_pc}'
                elif limit_start is not None and int(t.split(':',1)[1])==limit_start+2:body[i]=f'0branch:{limit_start}'
