...
def main():
    for path in (SOURCE, BUILDER, EXPECTED, NUCLEUS):
        if not path.is_file(): raise SystemExit(f"missing verification input: {path}")
    with tempfile.TemporaryDirectory() as td_name:
        td=Path(td_name); shutil.copy2(SOURCE,td/SOURCE.name)
        r=run(["python3",str(BUILDER)],cwd=td)
        if r.returncode: raise SystemExit(f"phase27 builder failed:\n{r.stderr}{r.stdout}")
        rebuilt=td/EXPECTED.name
        if rebuilt.read_bytes()!=EXPECTED.read_bytes(): raise SystemExit("phase27 generated artifact is not reproducible from the committed builder")
        cc=run(["cc","-std=c17","-Wall","-Wextra","-Wpedantic","-Werror",str(NUCLEUS),"-o",str(td/"nucleus")])
        if cc.returncode: raise SystemExit(f"nucleus strict build failed:\n{cc.stderr}")
        cases=[(": alpha 65 emit ;\n","A"),(": alpha 1 IF 65 emit ELSE 66 emit THEN ;\n","A"),(": alpha 0 IF 65 emit ELSE 66 emit THEN ;\n","B"),(": alpha 1 IF 1 IF 65 emit THEN ELSE 66 emit THEN ;\n","A")]
