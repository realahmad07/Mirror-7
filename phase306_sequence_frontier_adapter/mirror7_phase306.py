...
"""
    ...
    def evaluate(self, seed: int) -> float:
        return SealedSourceEvaluator().evaluate(
            self.engine.source, self._pack(seed)
        ).held_out

    def improve(self, seed: int, rounds: int, candidates: int) -> UpgradeOutcome:
        pack = self._pack(seed)
        report = self.engine.run_round(
            pack, "solver.py", ["sequence extrapolation deficit"], 1, candidates
        )
...
class SequenceFrontierDemo:
    ...
    def run(self, steps: int = 4):
        ...
        loop = FrontierUpgradeLoop(
            frontier, {"sequence_extrapolation": adapter}
        )
