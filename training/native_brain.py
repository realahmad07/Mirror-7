...
def build_native_brain(config: NativeBrainConfig):
    ...
    class NativeBrain(nn.Module):
        def __init__(self):
            ...
            self.gru = nn.GRU(config.embedding_dim, config.hidden_dim, num_layers=config.layers,
            ...
            self.transition = nn.Sequential(
                nn.Linear(config.state_dim, config.state_dim), nn.GELU(),
                nn.LayerNorm(config.state_dim), nn.Linear(config.state_dim, config.state_dim))
...
def train(args):
    ...
    if not train_items or not val_items: raise ValueError("dataset must contain train and validation splits")
    config = NativeBrainConfig(args.embedding_dim, args.hidden_dim, args.layers, args.state_dim,
                               args.dropout, args.max_bytes)
    ...
    def run_epoch(items, training):
        ...
        for start in range(0, len(order), args.batch_size):
            ...
            ti, tm = _batch([_encode(x.target_text, config.max_bytes) for x in batch], config.max_bytes)
            ui, um, ti, tm = ui.to(device), um.to(device), ti.to(device), tm.to(device)
            with torch.set_grad_enabled(training):
                ...
                if training:
                    ...
                    optimizer.step()
            totals[0] += float(loss.detach().cpu()); totals[1] += float(c_loss.cpu()); totals[2] += float(t_loss.cpu()); batches += 1
        return {"loss": totals[0]/max(1,batches), "contrastive": totals[1]/max(1,batches), "transition": totals[2]/max(1,batches)}
    ...
    torch.save({"format":"mirror7-native-brain-v1","config":config.to_dict(),"history":history,
                "device":str(device),"state_dict":model.state_dict()}, output)
    print(json.dumps({"output":str(output),"parameters":sum(p.numel() for p in model.parameters())}))

def main():
    p=argparse.ArgumentParser(description="Train Mirror 7 native semantic/state brain.")
    p.add_argument("--dataset",required=True); p.add_argument("--output",default="artifacts/mirror7_native_brain_v1.pt")
    p.add_argument("--epochs",type=int,default=5); p.add_argument("--batch-size",type=int,default=64)
    p.add_argument("--lr",type=float,default=3e-4); p.add_argument("--weight-decay",type=float,default=1e-2)
    p.add_argument("--grad-clip",type=float,default=1.0); p.add_argument("--embedding-dim",type=int,default=96)
    p.add_argument("--hidden-dim",type=int,default=256); p.add_argument("--layers",type=int,default=2)
    p.add_argument("--state-dim",type=int,default=128); p.add_argument("--dropout",type=float,default=0.10)
    p.add_argument("--max-bytes",type=int,default=512); p.add_argument("--temperature",type=float,default=0.07)
    p.add_argument("--seed",type=int,default=42); p.add_argument("--device",default=None)
    train(p.parse_args())
